...
function getInitialMode() {
  return location.pathname === "/login" || location.hash === "#login"
    ? "login"
    : "signup";
}
...
function redirectRequesterToDashboard(payload) {
  if (payload?.user?.role === "requester") {
...
signupForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (redirectRequesterToDashboard(result)) {
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (existingSession && existingSession.user && !isAuthRoute) {
    if (redirectRequesterToDashboard(existingSession)) {
      return;
    }
